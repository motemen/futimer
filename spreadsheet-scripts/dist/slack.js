...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/slack.ts");
/******/ })
/************************************************************************/
/******/ ({
...
function calcStats(results) {
    ...
    var scores = results.map(function (result) { return result.time; });
    return {
        averageOf: [5, 12, 100].reduce(function (a, n) {
            var _a;
            var aa = scores.length >= n ? variousAveragesOf(n, scores) : null;
            return __assign({}, a, (_a = {}, _a[n] = aa, _a));
        }, {}),
        single: {
            best: Math.min.apply(Math, scores),
            current: scores[scores.length - 1],
            worst: Math.max.apply(Math, scores)
        }
    };
}
...
function averageOf(scores) {
    var sorted = scores.slice().sort(function (a, b) { return (a < b ? 1 : a > b ? -1 : 0); });
    return meanOf(sorted.slice(1, sorted.length - 1));
}
function slicesFrom(n, array) {
    return new Array(array.length - n + 1)
        .fill(null)
        .map(function (_, i) { return array.slice(i, i + n); });
}
function variousAveragesOf(n, scores) {
    if (scores.length < n) {
        throw new Error("not enough scores: " + scores.length + " < " + n);
    }
    var _a = slicesFrom(n, scores)
        .map(function (s) { return averageOf(s); })
        .reduce(function (curr, v) {
        return {
            best: Math.min(v, curr.best),
            worst: Math.max(v, curr.worst)
        };
    }, { best: Number.MAX_VALUE, worst: Number.MIN_VALUE }), best = _a.best, worst = _a.worst;
    return {
        best: best,
...
exports.variousAveragesOf = variousAveragesOf;
function formatDuration(n) {
    if (n === undefined) {
        return undefined;
    }
    var subsecs = n.toString().split(/\./)[1] || "";
    var mins = Math.floor(n / 60);
    var secs = Math.floor(n % 60);
    var padLeft = function (s, c) {
        if (s.length === 2) {
            return s;
        }
        return c + s;
...
exports.__esModule = true;
var index_1 = __webpack_require__(/*! ../../src/models/index */ "../src/models/index.ts");
__webpack_require__(/*! es6-shim */ "./node_modules/es6-shim/es6-shim.js");
global.postStatsToSlack = function (e) {
    var props = PropertiesService.getScriptProperties();
    var WEBHOOK_URL = props.getProperty("SLACK_WEBHOOK_URL");
    if (!WEBHOOK_URL) {
        throw new Error("SLACK_WEBHOOK_URL script property not set");
    }
    var sheet = SpreadsheetApp.getActiveSheet();
    var lastRow = parseInt(props.getProperty("SLACK_LAST_ROW") || "", 10) || sheet.getLastRow();
    var range;
    try {
    ...
    var times = values.map(function (v) { return v[2]; });
    var session = values[0][0];
    var bestTime = Math.min.apply(Math, times);
    var ao5 = times.length >= 5 ? index_1.variousAveragesOf(5, times) : null;
    var ao12 = times.length >= 12 ? index_1.variousAveragesOf(12, times) : null;
    var payload = {
        username: "fuTimer",
        ...
        attachments: [
            {
                ...
                fields: [
                    ...
                    ["Best single", index_1.formatDuration(bestTime)],
                    ["Best ao5", ao5 && index_1.formatDuration(ao5.best)],
                    ["Best ao12", ao12 && index_1.formatDuration(ao12.best)]
                ]
                    .filter(function (_a) {
